...
Can four Linux namespaces behave like a tiny DN42-style network before BIRD or WireGuard are introduced?
...
- AS-shaped namespace: a namespace used as a toy autonomous system in the lab. It is not a real registered AS.
...
```mermaid
flowchart LR
  AS1["toy-as1<br/>service: 172.20.1.1/32"]
  AS2["toy-as2<br/>service: 172.20.2.1/32"]
  AS3["toy-as3<br/>service: 172.20.3.1/32"]
  AS4["toy-as4<br/>service: 172.20.4.1/32"]
...
The first selected path from `toy-as1` to `toy-as3` is clockwise:

```text
toy-as1 -> toy-as2 -> toy-as3
```

The backup physical path exists, but static routing will not use it until the route tables are changed:

```text
toy-as1 -> toy-as4 -> toy-as3
```
...
```text
experiments/labs/toy-dn42-static-routing/run.sh
```

The transcript used for this experiment is:

```text
experiments/transcripts/toy-dn42-static-routing-20260616T001904Z.txt
```
...
| --- | --- | --- | --- |
| Create AS-shaped namespaces | `ip netns add toy-as1` | Adds separate network stacks. | `ip netns list` shows four `toy-as*` namespaces. |
...
| Configure loopbacks | `ip -n toy-as1 addr add 172.20.1.1/32 dev lo` | Adds a stable service address inside a namespace. | Service addresses exist on `lo`. |
| Configure link addresses | `ip -n toy-as1 addr add 10.42.12.1/30 dev as1-as2` | Gives each local link an address pair. | Connected routes appear for each `/30` link. |
...
| Check baseline | `ip -n toy-as1 route get 172.20.3.1` | No route has been added yet. | Route lookup fails. |
| Add static routes | `ip -n toy-as1 route add 172.20.3.1/32 via 10.42.12.2 dev as1-as2` | Writes service reachability by hand. | Route lookup points to the selected next hop. |
| Test packet flow | `ping -I 172.20.1.1 172.20.3.1` | Sends packets between service loopbacks. | Ping succeeds. |
| Break selected link | `ip -n toy-as2 link set as2-as3 down` | Removes the link the static route depends on. | Ping fails even though another path exists. |
| Repair routes | `ip -n toy-as1 route replace ... via 10.42.41.1` | Points traffic at the alternate path. | Ping succeeds again. |
| Roll back | `ip netns delete toy-as1` | Deletes temporary namespaces. | Lab namespaces disappear. |
...
| --- | --- |
| `toy-as1` | `172.20.1.1/32` |
| `toy-as2` | `172.20.2.1/32` |
| `toy-as3` | `172.20.3.1/32` |
| `toy-as4` | `172.20.4.1/32` |

## Predict Before Running

Before static service routes exist, predict what `toy-as1` should do with a packet to `172.20.3.1`.

`toy-as1` has connected routes for its two local links:
...
The script installs clockwise static routes for traffic between `toy-as1` and `toy-as3`.
...
> To reach `toy-as3`'s service loopback, send packets to `toy-as2` over the `as1-as2` link.
...
> To keep forwarding toward `toy-as3`, send packets over the `as2-as3` link.
...
Forward path from `toy-as1` to `toy-as3`:

| Namespace doing lookup | Destination | Route needed |
| --- | --- | --- |
| `toy-as1` | `172.20.3.1/32` | `via 10.42.12.2 dev as1-as2` |
| `toy-as2` | `172.20.3.1/32` | `via 10.42.23.2 dev as2-as3` |

Reply path from `toy-as3` back to `toy-as1`:

| Namespace doing lookup | Destination | Route needed |
| --- | --- | --- |
| `toy-as3` | `172.20.1.1/32` | `via 10.42.23.1 dev as3-as2` |
| `toy-as2` | `172.20.1.1/32` | `via 10.42.12.1 dev as2-as1` |
...
The transcript shows nonzero RX and TX packet counts on both `toy-as2` transit links. These counters are supporting evidence, not a packet-perfect trace. They include the ping traffic plus other small link traffic such as neighbor-resolution packets. The important observation is that packets appeared on both transit interfaces while the service-loopback ping succeeded.

## Break the Selected Link

The script disables the selected `toy-as2` to `toy-as3` link:

```sh
ip -n toy-as2 link set as2-as3 down
```

`toy-as1` still has the old static route:

```text
172.20.3.1 from 172.20.1.1 via 10.42.12.2 dev as1-as2
```

That route still points at `toy-as2`. Static routing does not notice that the better next step is now the other side of the square.
...
After the route change, the selected path from `toy-as1` points to `toy-as4`:
...
- no Toy DN42 namespaces existed,
- no Toy DN42 veth links existed,
- no Toy DN42 service loopbacks existed,
...
- [ ] You can explain why `toy-as1` cannot reach `172.20.3.1` before static routes exist.
...
- Transcript: `experiments/transcripts/toy-dn42-static-routing-20260616T001904Z.txt`.
