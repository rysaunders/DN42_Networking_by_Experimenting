...
wait_for_route_gone() {
  ...
  birdc_ns "${ns}" show route all
  ip -n "${ns}" route
  printf 'Route %s remained in namespace %s\n' "${prefix}" "${ns}" >&2
  exit 1
}
...
section "Wait for BGP sessions"
wait_for_bgp "${AS1}" 2
wait_for_bgp "${AS2}" 2
wait_for_bgp "${AS3}" 2
wait_for_bgp "${AS4}" 2
run ip netns exec "${AS1}" birdc -s "$(bird_socket "${AS1}")" show protocols
run ip netns exec "${AS2}" birdc -s "$(bird_socket "${AS2}")" show protocols

section "Observe BIRD routes and kernel routes"
wait_for_route "${AS1}" "172.20.3.1"
run ip netns exec "${AS1}" birdc -s "$(bird_socket "${AS1}")" show route 172.20.3.1/32 all
...
section "Isolate AS3 and observe withdrawal"
run ip -n "${AS2}" link set as2-as3 down
run ip -n "${AS4}" link set as4-as3 down
wait_for_route_gone "${AS1}" "172.20.3.1"
run ip netns exec "${AS1}" birdc -s "$(bird_socket "${AS1}")" show protocols
run ip -n "${AS1}" route get 172.20.3.1 || true
run ip netns exec "${AS1}" ping -c 1 -W 1 -I 172.20.1.1 172.20.3.1 || true

section "Restore AS3 links and observe convergence"
run ip -n "${AS2}" link set as2-as3 up
run ip -n "${AS4}" link set as4-as3 up
wait_for_bgp "${AS2}" 2
wait_for_bgp "${AS3}" 2
...
section "Result"
printf 'Lab complete. Temporary namespaces and BIRD state removed.\n'
